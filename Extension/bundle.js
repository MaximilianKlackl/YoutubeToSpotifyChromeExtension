...
document.getElementById("playlist-icon-open").addEventListener("click", () => {
    document.getElementById("playlist-popup").style.cssText = "display: normal";
    document.getElementById("main-container").style.cssText = "display: none";
    console.log("t " + document.getElementById('playlist-popup').style.display);
    console.log("t " + document.getElementById('main-container').style.display);
})

document.getElementById("playlist-icon-close").addEventListener("click", () => {
    document.getElementById("playlist-popup").style.cssText = "display: none";
    document.getElementById("main-container").style.cssText = "display: normal";
})
...
function getPlaylistFromYoutube(){
    return new Promise((resolve, reject) => {
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            const port = chrome.tabs.connect(tabs[0].id);
            port.postMessage({ message: 'getPlaylist' });
            port.onMessage.addListener((response) => {
...
function convertPlaylist(){
    let playlist = getPlaylistFromYoutube();
    let uriList = [];

    playlist
    .then((arr) => {
        let uriList = getIds(arr);
        uriList
        .then((list) => {
            console.log(list);
            createPlaylist(list);
...
function getIds(arr){
    return new Promise((resolve) => {
        let uriList = [];
        arr.forEach(song => {
            if(song == arr[arr.length-1]){
                resolve(uriList);
            }
            spotifyApi.searchTracks(formatTitleForSearching(song))
            .then((res) => {
                if(res.tracks.items.length != 0){
                    uriList.push(res.tracks.items[0].uri);
                }
            });
        });
    })
...
function createPlaylist(uriList){
    spotifyApi.getMe()
    .then((res) => {
        const userId = res.id;
        spotifyApi.createPlaylist(userId, {
            ...
            "public":           "false", 
            "collaborative":    "false",
            "description":      "modify your description"
        })
        .then((res) => {
            console.log(res);
            ...
            spotifyApi.addTracksToPlaylist(playlistId, uriList)
            .then((res) => {
...
document.getElementById("url-link").addEventListener("click", () => {
    window.open(document.getElementById("url-link").href,'_blank');
})
